...
function InputContent() {
  ...
  const runModelOnImage = async (
    ...
    const tf = window.tf;
    if (!tflite || !tf) {
      throw new Error("AI engine is initializing. Please wait 2 seconds and try again.");
    }
    ...
    const predictedIndex = data.indexOf(maxVal);

    tensor.dispose();
    output.dispose();

    return { index: predictedIndex, confidence: maxVal };
  };
  ...
  const handleSubmit = async () => {
    ...
    if (image) {
      setLoading(true);
      try {
        const result = await runModelOnImage(image);
        confidence = result.confidence;
        // In predictor.py, confidence threshold is 0.75 (75%)
        if (confidence < 0.75) {
          predictedIndex = -2; // Uncertain / low confidence
        } else {
          predictedIndex = result.index;
        }
      } catch (err) {
        console.error(err);
    ...
    router.push(
      `/result?crop=${crop}&district=${district}&lang=${lang}&question=${encodeURIComponent(
        question
      )}&predictedIndex=${predictedIndex}&confidence=${confidence.toFixed(2)}`
    );
  };
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl shadow-green-100 border border-white p-7 w-full max-w-sm space-y-5">
        <div>
          ...
          <div className="relative">
            <textarea
              className="w-full border-2 border-gray-200 rounded-2xl p-3.5 pr-12 h-24 text-gray-900 font-medium placeholder:text-gray-400 bg-white focus:border-green-500 focus:outline-none transition-all resize-none shadow-sm"
