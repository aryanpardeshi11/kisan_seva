      ...
      {showAddActivity && (
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl shadow-green-100 border border-white p-6 mb-6 space-y-4">
          <div>
            ...
            <input type="date" className="w-full border-2 border-gray-200 rounded-2xl p-3 text-gray-900 font-medium bg-white focus:border-green-500 focus:outline-none shadow-sm" value={actDate} onChange={(e) => setActDate(e.target.value)} />
          </div>
          <div>
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Type</label>
            <select className="w-full border-2 border-gray-200 rounded-2xl p-3 text-gray-900 font-medium bg-white focus:border-green-500 focus:outline-none shadow-sm" value={actType} onChange={(e) => setActType(e.target.value)}>
              {activityTypes.map((t) => (
          ...
          <div>
            ...
            <input type="text" className="w-full border-2 border-gray-200 rounded-2xl p-3 text-gray-900 font-medium placeholder:text-gray-400 bg-white focus:border-green-500 focus:outline-none shadow-sm" placeholder="e.g. Urea 50kg spray" value={actNote} onChange={(e) => setActNote(e.target.value)} />
          </div>
          <div>
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Cost (₹)</label>
            <input type="number" className="w-full border-2 border-gray-200 rounded-2xl p-3 text-gray-900 font-medium placeholder:text-gray-400 bg-white focus:border-green-500 focus:outline-none shadow-sm" value={actCost} onChange={(e) => setActCost(e.target.value)} />
          </div>
          <div>
            <label className="text-sm font-semibold text-gray-700 mb-2 block">Photo (optional)</label>
            <input
              type="file"
              accept="image/*"
              className="w-full text-xs text-gray-600 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-green-100 file:text-green-700 hover:file:bg-green-200 cursor-pointer"
              onChange={(e) => setActPhoto(e.target.files?.[0] || null)}
            />
          </div>
      ...
      {showAddRevenue && (
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl shadow-green-100 border border-white p-6 mb-6 space-y-4">
          <div>
            ...
            <input type="number" className="w-full border-2 border-gray-200 rounded-2xl p-3 text-gray-900 font-medium placeholder:text-gray-400 bg-white focus:border-green-500 focus:outline-none shadow-sm" value={revenueAmount} onChange={(e) => setRevenueAmount(e.target.value)} />
          </div>
